	...
	function init_redis_setup() {
	    function checkConfig() {
	        ...
	        if (external) {
	            ...
	            var host = jquery.trim(String(jquery('#host').val()));
	            if (!host) {
	                jquery('#err_msg_host').html('Redis服务器地址不正确');
	                ok = false;
	            }
	            else {
	                jquery('#err_msg_host').html('');
	            ...
	            var port = jquery('#port').val();
	            if (!port || isNaN(parseInt(String(port), 10))) {
	                jquery('#err_msg_port').html('Redis服务器端口不正确');
	                ok = false;
	            }
	            else {
	                jquery('#err_msg_port').html('');
	    ...
	    jquery('#ok').on('click', function (e) {
	        e.preventDefault();
	        if (checkConfig()) {
	            jquery('#setup-redis-form').submit();
	        }
	    });
	}
	exports.init_redis_setup = init_redis_setup;

	});

	unwrapExports(redis);
	var redis_1 = redis.init_redis_setup;
