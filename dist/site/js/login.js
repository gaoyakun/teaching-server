(function (global, factory) {
	...
	(factory((global.login = {}),global.jQuery));
	...
	function login_setup() {
	    function checkConfig() {
	        ...
	        if (!account) {
	            jquery('#err_msg_account').html('账号不正确');
	            ok = false;
	        }
	        else {
	        ...
	        if (checkConfig()) {
	            jquery.ajax({
	                ...
	                data: {
	                    ...
	                    md5password: md5_1.md5(String(jquery('#password').val())),
	                    remember: jquery('#remember-checkbox').prop('checked') ? 1 : 0
	                },
	                dataType: 'json',
	...
	exports.login_setup = login_setup;

	});

	unwrapExports(login);
	var login_1 = login.login_setup;
	...
	function register_setup() {
	    function checkConfig() {
	        ...
	        if (!email) {
	            jquery('#err_msg_email').html('邮箱不正确');
	            ok = false;
	        }
	        else {
	        ...
	        if (password2 !== password) {
	            jquery('#err_msg_password2').html('密码不匹配');
	            ok = false;
	        }
	        else {
	...
	exports.register_setup = register_setup;

	});

	unwrapExports(register);
	var register_1 = register.register_setup;
	...
	var login$3 = unwrapExports(login$2);
	var login_2 = login$2.Login;

	exports.default = login$3;
	exports.Login = login_2;
