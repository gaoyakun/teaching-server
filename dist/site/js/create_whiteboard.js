(function (global, factory) {
	    ...
	    WBEditorToolbox.prototype.loadTools = function (tools) {
	        ...
	        tools.forEach(function (tool) {
	            ...
	            if (tool.activeCommand === '$StrokeColor') {
	                ...
	                inputBox_1.type = 'color';
	                inputBox_1.value = _this._strokeColor;
	                inputBox_1.style.width = buttonSize + "px";
	                inputBox_1.style.height = buttonSize + "px";
	                inputBox_1.style.margin = '2px';
	                inputBox_1.onchange = function () {
	                    _this._strokeColor = inputBox_1.value;
	                };
	                _this._container.appendChild(inputBox_1);
	            }
	            else if (tool.activeCommand === '$FillColor') {
	                var inputBox_2 = document.createElement('input');
	                inputBox_2.type = 'color';
	                inputBox_2.value = _this._fillColor;
	                inputBox_2.style.width = buttonSize + "px";
	                inputBox_2.style.height = buttonSize + "px";
	                inputBox_2.style.margin = '2px';
	                inputBox_2.onchange = function () {
	                    _this._fillColor = inputBox_2.value;
	                };
	                _this._container.appendChild(inputBox_2);
	            }
	            else {
	                var toolButton_1 = document.createElement('div');
	                toolButton_1.classList.add('flex-h', 'flex-align-x-center', 'flex-align-y-center');
	                tool.elementId = "toolbutton-" + WBEditorToolbox.uniqueId++;
	                toolButton_1.classList.add('toolbutton');
	                toolButton_1.id = tool.elementId;
	                toolButton_1.style.width = buttonSize + "px";
	                toolButton_1.style.height = buttonSize + "px";
	                toolButton_1.setAttribute('toolIndex', String(_this._tools.length - 1));
	                var toolIcon_1 = document.createElement('i');
	                toolIcon_1.style.fontSize = tool.fontSize || '60px';
	                toolIcon_1.style.color = '#fff';
	                toolIcon_1.style.lineHeight = tool.fontSize || '60px';
	                tool.iconClass && tool.iconClass.split(' ').forEach(function (cls) {
	                    toolIcon_1.classList.add(cls);
	                });
	                toolButton_1.appendChild(toolIcon_1);
	                _this._container.appendChild(toolButton_1);
	                toolButton_1.addEventListener('click', function () {
	                    var toolIndex = Number(toolButton_1.getAttribute('toolIndex'));
	                    ...
	                    if (tool !== _this._curTool) {
	                        if (_this._curTool) {
	                            ...
	                            if (_this._curTool.deactiveCommand) {
	                                _this._wb.executeCommand(commands.WBCommandParser.parse(_this._curTool.deactiveCommand));
	                            }
	                            _this._curTool = null;
	                        }
	                    }
	                    if (tool) {
	                        ...
	                        _this._wb.executeCommand(commands.WBCommandParser.parse(tool.activeCommand));
	                        _this._curTool = tool;
	                    }
	                });
	            }
	...
	var WBTool = /** @class */ (function (_super) {
	    ...
	    WBTool.prototype.executeCommand = function (cmd) {
	    };
	    return WBTool;
	}(catk.EventObserver));
	...
	var WhiteBoard = /** @class */ (function (_super) {
	    ...
	    WhiteBoard.prototype.encodeCommand = function (cmd) {
	        var str = cmd.command;
	        for (var name_1 in cmd) {
	            if (name_1 !== 'command') {
	                str += " " + name_1 + "=" + cmd[name_1];
	            }
	        }
	        return str;
	    };
	    WhiteBoard.prototype.executeCommand = function (cmd) {
	        var _this = this;
	        ...
	        if (cmd.command === 'UseTool') {
	            ...
	            var type = cmd.type;
	            var name_2 = cmd.name || null;
	            var failOnExists = !!cmd.failOnExists;
	            var params = cmd.params || {};
	            var obj = this.createEntity(type, name_2, failOnExists, cmd.x, cmd.y, params);
	            ...
	            var obj = this.findEntity(cmd.objectName);
	            if (obj) {
	                var ev = new WBGetPropertyEvent(cmd.propName);
	                obj.triggerEx(ev);
	...
	var whiteboard_1 = whiteboard.WBComponent;
	var whiteboard_2 = whiteboard.WBFactory;
	var whiteboard_3 = whiteboard.WBToolActivateEvent;
	var whiteboard_4 = whiteboard.WBToolDeactivateEvent;
	var whiteboard_5 = whiteboard.WBGetPropertyListEvent;
	var whiteboard_6 = whiteboard.WBSetPropertyEvent;
	var whiteboard_7 = whiteboard.WBGetPropertyEvent;
	var whiteboard_8 = whiteboard.WBGetObjectEvent;
	var whiteboard_9 = whiteboard.WBTool;
	var whiteboard_10 = whiteboard.WhiteBoard;
	...
	var WBToolPalette = /** @class */ (function () {
	                ...
	                toolButton.addEventListener('click', function () {
	                    ...
	                    if (tool !== _this._curTool) {
	                        if (_this._curTool) {
	                            var curToolButton = document.querySelector("#" + _this._curTool.elementId);
	                            curToolButton && curToolButton.classList.remove('active');
	                            _this._editor.executeCommand({ command: 'UseTool' });
	                            _this._curTool = null;
	                        }
	                    }
	                    if (tool) {
	                        var button = document.querySelector("#" + tool.elementId);
	                        button && button.classList.add('active');
	                        _this._editor.executeCommand(tool.command);
	                        _this._curTool = tool;
	                    }
	                });
	            }
	    ...
	    WBToolPalette.prototype.getOpTool = function (tool, name) {
	        ...
	        var cmd = tool[name].command;
	        if (cmd) {
	            tooldef.command.command = cmd;
	        }
	        var args = tool[name].args;
	        if (args) {
	        ...
	        if (view) {
	            ...
	            this.addTextAttribute('页面名称', view.currentPage, false, function (value) {
	                if (value !== view.currentPage) {
	                    _this._editor.whiteboard.executeCommand({
	                        command: 'RenamePage',
	                        newName: value
	                    });
	                    _this.loadPageProperties();
	...
	var WBEditor = /** @class */ (function () {
	    function WBEditor(whiteboard$$1, toolset, toolPaletteElement, opPaletteElement, objectPropGridElement, toolPropGridElement) {
	        this._strokeColor = '#00000000';
	        this._fillColor = 'red';
	        this._toolFontSize = 14;
	        this._wb = whiteboard$$1;
	    ...
	    WBEditor.prototype.executeCommand = function (cmd) {
	        if (typeof cmd.command === 'function') {
	            ...
	            var realCommand = {};
	            for (var name_1 in cmd) {
	                var value = cmd[name_1];
	                realCommand[name_1] = (typeof value === 'function') ? value(this) : value;
	            }
	    ...
	    tools: {
	        ...
	        Write: {
	            ...
	            command: function (editor) {
	                var cmd = { command: 'UseTool', name: 'HandWriting', args: { mode: 'draw' } };
	        ...
	        Erase: {
	            ...
	            command: function (editor) {
	                var cmd = { command: 'UseTool', name: 'HandWriting', args: { mode: 'erase' } };
	                editor.whiteboard.executeCommand(cmd);
	            }
	        }
	    },
	...
	var WBSelectTool = /** @class */ (function (_super) {
	    ...
	    WBSelectTool.prototype.executeCommand = function (cmd) {
	        ...
	        this.on(catk.EvtMouseDown.type, function (ev) {
	            ...
	            for (var arg in _this.options) {
	                if (arg !== 'command' && arg !== 'createType' && arg !== 'type') {
	                    cmd[arg] = _this.options[arg];
	                }
	            }
	            cmd.x = ev.x;
	            cmd.y = ev.y;
	            cmd.params = _this._creationParams[_this.options.createType];
	            _this._wb.executeCommand(cmd);
	        });
	...
	var WBConnectTool = /** @class */ (function (_super) {
	    ...
	    WBConnectTool.prototype.executeCommand = function (cmd) {
	    };
